...
\begin{frame}{Was m\"ochten wir?}
	\begin{block}{Das Problem}
		Spezifische Textdateien (z.B. Quellcode) zwischen Maschinen synchronisieren und sie mit mehreren Leuten
		bearbeiten. 
	\end{block}
	\pause
...
\begin{frame}{Was ist git?}
	\begin{block}{Was brauchen wir}
	\begin{itemize}
		\item Synchronisation
		\item Team-Dateibearbeitung
		\item Problemlose Offline-Nutzung
	\end{itemize}
	\end{block}
	\pause

	\begin{block}{Was kriegen wir zusätzlich}
	\begin{itemize}
		\item Geschichte jedes Dokuments im Projekt
		\item Kryptographische Sicherheit der Projektgeschichte
		\item Gemeinsamer Dateizugriff ohne zentralen Server 
	\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}[c]{Bemerkung}
	\centering
	{\LARGE  
	Git l\"ost ein komplexes Problem, \\
...
\pause
F\"ur Git ist ein \emph{Repo} ein Verzeichnis mit einem speziellen (unsichtbaren)
Unterverzeichnis \texttt{\textcolor{red}{.git}}

\vspace{1em}

Man sollte \textbf{nie} \texttt{.git} l\"oschen.
\end{minipage}
\end{frame}
...
\begin{frame}{Begriff: Commit}
	...
	\begin{center}
	\textbf{
		Commits \textcolor{red}{k\"onnen nicht} ge\"andert werden, \\
...
\begin{frame}{Begriff: Remote}\begin{center}
	...
	\begin{tikzpicture}
		...
		\draw[blue!80, fill=blue!10]
			...
			]{Laptop\\{\footnotesize (User)}} rectangle ($(DB1.south east) + (1, -1)$);
	...
	Ein \emph{Remote} ist ein Clone (Kopie) des Repos auf einer anderen Maschine.
	\\
	\pause

	Remote k\"onnen einen Namen haben, z.B. \texttt{origin}.
	\end{minipage}
\end{center}
\end{frame}
...
\begin{frame}[fragile]{Konflikte}
	\begin{block}{}
	Was passiert wenn 2 Leute die gleiche Linie in einem Dokument bearbeiten?
	\\
	\pause
	\vspace{1em}
	Git kann nicht wissen, welche Version besser ist, weshalb der User gefragt wird.
	\end{block}
	\begin{verbatim}
...
...
\begin{frame}{Arbeitsablauf}
	...
	\begin{enumerate}
		\item Forke ein Repository von HSR-Stud \pause
		\item Bearbeiten \pause
		\item Fortlaufend Commits erstellen, z.B. nach jedem Kapitel \pause
		\item Push
	\end{enumerate}
\end{frame}
...
\begin{enumerate}
	...
	\item Bearbeiten \pause
	\item Fortlaufend Commits erstellen, z.B. nach jedem Kapitel \pause
	\item Mit dem Master mergen, z.B. nach jedem Kapitel \pause
	\item Push
\end{enumerate}
\end{frame}
